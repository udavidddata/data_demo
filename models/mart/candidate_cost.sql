...
communication_costs as (
   select 
       ...
       sum(case when support_oppose_indicator = 'S' then total else 0 end) as supporting_costs,
       sum(case when support_oppose_indicator = 'O' then total else 0 end) as opposing_costs
...
from candidates c
left join communication_costs cc 
